...
const getCurrentSlowOpsTool: Tool = {
...
const analyzeProfilerLogsTool: Tool = {
...
const checkReplicationStatusTool: Tool = {
...
function getClient(request: any): MongoClient {
  const args = request.params.arguments as { cluster_name?: string };
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  try {
    ...
    if (request.params.name === "get_current_slow_ops") {
      const args = request.params.arguments as { thresholdSeconds?: number };
      ...
      try {
        ...
        if (primary && primary.optimeDate) {
           ...
           if (!isNaN(primaryTime)) {
             lagInfo = members.map((m: any) => {
                if (m.stateStr === "PRIMARY") return `${m.name}: PRIMARY`;
                if (!m.optimeDate) return `${m.name}: No optimeDate`;
